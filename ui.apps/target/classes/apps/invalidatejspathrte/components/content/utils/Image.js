...
use(["/libs/wcm/foundation/components/utils/ResourceUtils.js", "/libs/sightly/js/3rd-party/q.js"], function (ResourceUtils, Q) {
    ...
    var CONST = {
        ...
        WEB_RENDITION_ORIGINAL: "original",
    ...
    Image.prototype.linkUrl = function () {
        var linkUrlPromise = Q.defer();
        var linkUrl = that.properties[CONST.PROP_LINK_URL];
        if(linkUrl) {
            // in case linkUrl is an internal anchor or external url
            if(linkUrl.indexOf("#") === 0 || linkUrl.indexOf("/content") !== 0) {
                linkUrlPromise.resolve(linkUrl);
            } else {
                var slashPos = linkUrl.indexOf("#"),
                    dotPos = linkUrl.indexOf(".");
    ...
    function getExtension() {
        ...
        if (fileReference) {
            ...
            log.debug("Using local image mime type");
            ResourceUtils.getResource(that.resource.path + "/" + CONST.NODE_NAME_FILE + "/" + CONST.PROP_JCR_CONTENT)
                .then(function (fileContentResource) {
                    var mimeType = fileContentResource.properties[CONST.PROP_JCR_MIME_TYPE] || defaultMimeType;
                    log.debug("Found mime type for image " + that.resource.path + " : " + mimeType);
                    extensionPromise.resolve(CONST.MIME_TYPES[mimeType]);
                }, function (error) {
                    log.error(error);
                });
        }
    ...
    function searchWebRendition(renditions, currentIndex, promise) {
        ...
        if (currentRendition.name.indexOf(CONST.WEB_RENDITION_PREFIX) >= 0
                || currentRendition.name.equals(CONST.WEB_RENDITION_ORIGINAL)) {
